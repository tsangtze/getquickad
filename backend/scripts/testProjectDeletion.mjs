...
async function fixture(run, overrides={}) {
 ...
 const create=vm.runInNewContext(source+'\n;createProjectRouter',{fs,path,console,URL,crypto:{},express:{Router:()=>router},multer,cookieParser:()=> (_q,_s,next)=>next(),requireUser(q,s,next){if(!q.authUser)return s.status(401).json({ok:false});next();},authConfiguration:()=>({applicationOrigin:'http://localhost:4100'}),prepareMusic:()=>({metadata:{id:'none'}}),validateMusicVolume:()=>10,validateStoryboard:s=>({ok:true,storyboard:s}),getUserUsage:async()=>({finalVideoCount:0,planId:'free',monthlyCreditsUsed:0}),canGenerateFinalVideo:()=>({ok:true,freeRerender:false}),recordSuccessfulFinalVideo:async()=>({planId:'free',creditCost:0}),LIMITS:{FREE_FINAL_VIDEOS:2,MAX_PROJECTS:10,FREE_MAX_VIDEO_SECONDS:30,PAID_MAX_VIDEO_SECONDS:60},generateNarration:async()=>({}),renderVideo:async()=>({}),uploadToR2:async()=>{throw new Error('R2 disabled in deletion test');},...overrides});
...
test('cancel never requests deletion; success refreshes; failure displays error',async()=>{let confirmed=false,calls=0,refreshes=0,ok=true;const status={textContent:'',classList:{add(){}}};const context={window:{confirm:()=>confirmed,location:{reload(){}}},currentProjectId:'',encodeURIComponent,recentProjectStatus:status,uiText:(_key,fallback,params={})=>String(fallback).replace(/\{([^{}]+)\}/g,(_match,name)=>Object.prototype.hasOwnProperty.call(params,name)?String(params[name]):_match),localizedApiError:error=>error?.error||'',quickAdProjectFetch:async(url,options)=>{calls++;assert.equal(options.method,'DELETE');return {ok,json:async()=>({ok,error:'Busy'})};},loadAccountProjects:async()=>refreshes++};const remove=vm.runInNewContext(deletion+';deleteSavedProject',context);const button={};await remove({id,title:'Test'},button);assert.equal(calls,0);confirmed=true;await remove({id,title:'Test'},button);assert.equal(refreshes,1);ok=false;await remove({id,title:'Test'},button);assert.equal(status.textContent,'Busy');assert.equal(button.disabled,false);});
