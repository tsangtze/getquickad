...
export function createPasswordHandlers({ createAuthClient, authConfiguration, fetchImpl = fetch }) {
  const accepted = { ok: true, message: "If this address is eligible, a password email will be sent. Check your inbox and spam folder, and use the newest link. A previous request may already have sent one." };
  const invalidLink = { ok: false, error: "This link is invalid or expired. Request a new password email below." };
  const unavailable = { ok: false, error: "Password service is temporarily unavailable. Please try again later." };
  ...
  return {
    ...
    async update(request, response) {
      ...
      if (typeof password !== "string" || password.length < 12 || password.length > 1024) {
        return response.status(400).json({ ok: false, error: "Choose a unique password of 12–1024 characters." });
      }
      try {
        const user = await verifiedUser(request, response);
        if (!user) return;
        ...
        if (!result.ok) {
          ...
          if (["weak_password", "same_password"].includes(data.code || data.error_code)) {
            return response.status(400).json({ ok: false, error: "Choose a stronger password that differs from your current password." });
          }
          if (result.status === 429) return response.status(429).json({ ok: false, error: "Too many password attempts. Please wait before trying again." });
          return response.status(503).json(unavailable);
        }
        ...
        return response.json({ ok: true, message: "Password saved. Return to QuickAd AI and sign in with your new password." });
      } catch {
        return response.status(503).json({ ok: false, error: "The password change could not be confirmed. Try signing in with your new password before requesting another link." });
      }
    }
  };
}
